...
-type valid_message(OTH) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, OTH}.

-type websocket_message(OTI) :: {valid, valid_message(OTI)} | invalid.
...
-type handler_message(OTJ) :: {internal, gramps@websocket:frame()} | {user, OTJ}.

-type websocket_state(OTK) :: {websocket_state,
        bitstring(),
        OTK,
        gleam@option:option(gramps@websocket@compression:compression())}.

-file("/Users/amanning/code/mist/src/mist/internal/websocket.gleam", 56).
...
-file("/Users/amanning/code/mist/src/mist/internal/websocket.gleam", 264).
-spec get_messages(
    bitstring(),
...
-file("/Users/amanning/code/mist/src/mist/internal/websocket.gleam", 351).
...
-file("/Users/amanning/code/mist/src/mist/internal/websocket.gleam", 358).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(OUS))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(OUS))).
...
-file("/Users/amanning/code/mist/src/mist/internal/websocket.gleam", 277).
-spec apply_frames(
    list(gramps@websocket:frame()),
    fun((OUI, websocket_connection(), handler_message(OUJ)) -> gleam@otp@actor:next(OUJ, OUI)),
    websocket_connection(),
    gleam@otp@actor:next(websocket_message(OUJ), OUI),
    fun((OUI) -> nil)
) -> gleam@otp@actor:next(websocket_message(OUJ), OUI).
...
-file("/Users/amanning/code/mist/src/mist/internal/websocket.gleam", 82).
-spec initialize_connection(
    fun((websocket_connection()) -> {OTT,
        gleam@option:option(gleam@erlang@process:selector(OTU))}),
    fun((OTT) -> nil),
    fun((OTT, websocket_connection(), handler_message(OTU)) -> gleam@otp@actor:next(OTU, OTT)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok, gleam@erlang@process:subject(websocket_message(OTU))} | {error, nil}.
