...
-opaque builder(TSW, TSX) :: {builder,
        integer(),
        fun((gleam@http@request:request(TSW)) -> gleam@http@response:response(TSX)),
...
-type websocket_message(TSY) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, TSY}.
...
-spec new(
    fun((gleam@http@request:request(TTV)) -> gleam@http@response:response(TTX))
) -> builder(TTV, TTX).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(TUB, TUC), integer()) -> builder(TUB, TUC).
port(Builder, Port) ->
    _record = Builder,
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), TUH),
    integer(),
    gleam@http@response:response(TUH)
) -> builder(mist@internal@http:connection(), TUH).
...
-spec bind(builder(TUT, TUU), binary()) -> builder(TUT, TUU).
...
-spec with_ipv6(builder(TUZ, TVA)) -> builder(TUZ, TVA).
with_ipv6(Builder) ->
    _record = Builder,
    {builder,
        erlang:element(2, _record),
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(TWA)
) -> {ok, websocket_message(TWA)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((TWG, mist@internal@websocket:websocket_connection(), websocket_message(TWH)) -> gleam@otp@actor:next(TWH, TWG)),
    fun((mist@internal@websocket:websocket_connection()) -> {TWG,
        gleam@option:option(gleam@erlang@process:selector(TWH))}),
    fun((TWG) -> nil)
...
-spec server_sent_events(
    ...
    fun(() -> gleam@otp@actor:init_result(TWV, TWW)),
    fun((TWW, sse_connection(), TWV) -> gleam@otp@actor:next(TWW, TWV))
) -> gleam@http@response:response(response_data()).
