...
-type valid_message(TFZ) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, TFZ}.

-type websocket_message(TGA) :: {valid, valid_message(TGA)} | invalid.
...
-type handler_message(TGB) :: {internal, gramps@websocket:frame()} | {user, TGB}.

-type websocket_state(TGC) :: {websocket_state,
        bitstring(),
        TGC,
...
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(THK))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(THK))).
...
-spec apply_frames(
    ...
    fun((THA, websocket_connection(), handler_message(THB)) -> gleam@otp@actor:next(THB, THA)),
    websocket_connection(),
    gleam@otp@actor:next(websocket_message(THB), THA),
    fun((THA) -> nil)
) -> gleam@otp@actor:next(websocket_message(THB), THA).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {TGL,
        gleam@option:option(gleam@erlang@process:selector(TGM))}),
    fun((TGL) -> nil),
    fun((TGL, websocket_connection(), handler_message(TGM)) -> gleam@otp@actor:next(TGM, TGL)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok, gleam@erlang@process:subject(websocket_message(TGM))} | {error, nil}.
