...
-spec map_response_body(
    fun((DVX) -> gleam@http@response:response(DVJ)),
    fun((DVJ) -> DVK)
) -> fun((DVX) -> gleam@http@response:response(DVK)).
...
-spec prepend_response_header(
    fun((DWB) -> gleam@http@response:response(DWD)),
    binary(),
    binary()
) -> fun((DWB) -> gleam@http@response:response(DWD)).
...
-spec ensure_post(gleam@http@request:request(DVO)) -> {ok,
        gleam@http@request:request(DVO)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(DXK)) -> DXS)) -> fun((gleam@http@request:request(DXK)) -> DXS).
