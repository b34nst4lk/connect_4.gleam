...
-spec set_header(request(DHH), binary(), binary()) -> request(DHH).
...
-spec prepend_header(request(DHK), binary(), binary()) -> request(DHK).
...
-spec set_body(request(any()), DHP) -> request(DHP).
set_body(Req, Body) ->
...
-spec map(request(DHR), fun((DHR) -> DHT)) -> request(DHT).
...
-spec set_query(request(DID), list({binary(), binary()})) -> request(DID).
set_query(Req, Query) ->
    Pair = fun(T) ->
...
-spec set_method(request(DIH), gleam@http:method()) -> request(DIH).
...
-spec set_scheme(request(DIO), gleam@http:scheme()) -> request(DIO).
...
-spec set_host(request(DIR), binary()) -> request(DIR).
set_host(Req, Host) ->
    _record = Req,
    {request,
        erlang:element(2, _record),
...
-spec set_port(request(DIU), integer()) -> request(DIU).
set_port(Req, Port) ->
    _record = Req,
    {request,
        erlang:element(2, _record),
...
-spec set_path(request(DIX), binary()) -> request(DIX).
set_path(Req, Path) ->
    _record = Req,
    {request,
        erlang:element(2, _record),
...
-spec set_cookie(request(DJA), binary(), binary()) -> request(DJA).
...
-spec remove_cookie(request(DJF), binary()) -> request(DJF).
