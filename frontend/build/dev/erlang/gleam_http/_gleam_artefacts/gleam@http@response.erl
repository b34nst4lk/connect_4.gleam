...
-type response(DRI) :: {response, integer(), list({binary(), binary()}), DRI}.
...
-spec set_header(response(DRX), binary(), binary()) -> response(DRX).
...
-spec prepend_header(response(DSA), binary(), binary()) -> response(DSA).
...
-spec set_body(response(any()), DSF) -> response(DSF).
...
-spec try_map(response(DRJ), fun((DRJ) -> {ok, DRL} | {error, DRM})) -> {ok,
        response(DRL)} |
    {error, DRM}.
...
-spec map(response(DSH), fun((DSH) -> DSJ)) -> response(DSJ).
...
-spec expire_cookie(response(DSR), binary(), gleam@http@cookie:attributes()) -> response(DSR).
