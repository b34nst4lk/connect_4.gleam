...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, BCC} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, BCC} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, BBW} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, BBW} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, BBK} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, BBK} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(BCG)) -> {ok, BCG} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(BBS)) -> {ok, BBS} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(BBO)) -> {ok, BBO} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(BCM), fun((BCM) -> json())) -> json().
...
-spec array(list(BCQ), fun((BCQ) -> json())) -> json().
