...
-type spec(ESL, ESM) :: {spec,
        ESL,
        integer(),
        integer(),
        fun((children(ESL)) -> children(ESM))}.

-opaque children(ESN) :: {ready, starter(ESN)} | {failed, child_start_error()}.

-opaque child_spec(ESO, ESP, ESQ) :: {child_spec,
        fun((ESP) -> {ok, gleam@erlang@process:subject(ESO)} |
            {error, gleam@otp@actor:start_error()}),
        fun((ESP, gleam@erlang@process:subject(ESO)) -> ESQ)}.
...
-spec start_child(child_spec(any(), ESX, ESY), ESX) -> {ok, child(ESY)} |
    {error, child_start_error()}.
...
-spec perform_instruction_for_child(
    ETL,
    instruction(),
    child_spec(any(), ETL, ETN),
    child(ETN)
) -> {ok, {child(ETN), instruction()}} | {error, child_start_error()}.
...
-spec start_and_add_child(starter(EUE), child_spec(any(), EUE, EUH)) -> children(EUH).
...
-spec add(children(EUM), child_spec(any(), EUM, EUP)) -> children(EUP).
...
-spec supervisor(
    fun((EUU) -> {ok, gleam@erlang@process:subject(EUV)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(EUV, EUU, EUU).
supervisor(Start) ->
...
-spec worker(
    fun((EVC) -> {ok, gleam@erlang@process:subject(EVD)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(EVD, EVC, EVC).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.
...
-spec returning(
    child_spec(EVK, EVL, any()),
    fun((EVL, gleam@erlang@process:subject(EVK)) -> EVR)
) -> child_spec(EVK, EVL, EVR).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-file("src/gleam/otp/supervisor.gleam", 248).
-spec init(spec(any(), EVW)) -> gleam@otp@actor:init_result(state(EVW), message()).
init(Spec) ->
...
-spec handle_exit(gleam@erlang@process:pid_(), state(EWC)) -> gleam@otp@actor:next(message(), state(EWC)).
...
-spec loop(message(), state(EWH)) -> gleam@otp@actor:next(message(), state(EWH)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
