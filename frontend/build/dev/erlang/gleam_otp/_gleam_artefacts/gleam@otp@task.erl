...
-opaque task(FFU) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:subject(FFU)}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message2(FFV, FFW) :: {m2_from_subject1, FFV} |
    {m2_from_subject2, FFW} |
    m2_timeout.

-type message3(FFX, FFY, FFZ) :: {m3_from_subject1, FFX} |
    {m3_from_subject2, FFY} |
    {m3_from_subject3, FFZ} |
    m3_timeout.

-type message4(FGA, FGB, FGC, FGD) :: {m4_from_subject1, FGA} |
    {m4_from_subject2, FGB} |
    {m4_from_subject3, FGC} |
    {m4_from_subject4, FGD} |
    m4_timeout.

-type message(FGE) :: {message, integer(), FGE} | message_timeout.
...
-spec try_await(task(FGJ), integer()) -> {ok, FGJ} | {error, await_error()}.
...
-spec await(task(FGN), integer()) -> FGN.
await(Task, Timeout) ->
...
-spec await_forever(task(FGR)) -> FGR.
await_forever(Task) ->
    assert_owner(Task),
    Selector = begin
        _pipe = gleam_erlang_ffi:new_selector(),
...
-spec try_await2_loop(
    gleam@erlang@process:selector(message2(FHB, FHC)),
    gleam@option:option({ok, FHB} | {error, await_error()}),
    gleam@option:option({ok, FHC} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, FHB} | {error, await_error()}, {ok, FHC} | {error, await_error()}}.
...
-spec try_await2(task(FGT), task(FGV), integer()) -> {{ok, FGT} |
        {error, await_error()},
    {ok, FGV} | {error, await_error()}}.
...
-spec try_await3_loop(
    gleam@erlang@process:selector(message3(FIC, FID, FIE)),
    gleam@option:option({ok, FIC} | {error, await_error()}),
    gleam@option:option({ok, FID} | {error, await_error()}),
    gleam@option:option({ok, FIE} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, FIC} | {error, await_error()},
    {ok, FID} | {error, await_error()},
    {ok, FIE} | {error, await_error()}}.
...
-spec try_await3(task(FHQ), task(FHS), task(FHU), integer()) -> {{ok, FHQ} |
        {error, await_error()},
    {ok, FHS} | {error, await_error()},
    {ok, FHU} | {error, await_error()}}.
...
-spec try_await4_loop(
    gleam@erlang@process:selector(message4(FJO, FJP, FJQ, FJR)),
    gleam@option:option({ok, FJO} | {error, await_error()}),
    gleam@option:option({ok, FJP} | {error, await_error()}),
    gleam@option:option({ok, FJQ} | {error, await_error()}),
    gleam@option:option({ok, FJR} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, FJO} | {error, await_error()},
    {ok, FJP} | {error, await_error()},
    {ok, FJQ} | {error, await_error()},
    {ok, FJR} | {error, await_error()}}.
...
-spec try_await4(task(FIY), task(FJA), task(FJC), task(FJE), integer()) -> {{ok,
            FIY} |
        {error, await_error()},
    {ok, FJA} | {error, await_error()},
    {ok, FJC} | {error, await_error()},
    {ok, FJE} | {error, await_error()}}.
...
-spec dict_to_list(gleam@dict:dict(integer(), FLH), integer(), FLH) -> list(FLH).
...
-spec try_await_all_loop(
    gleam@dict:dict(integer(), {ok, FKX} | {error, await_error()}),
    integer(),
    gleam@erlang@process:timer(),
    gleam@erlang@process:selector(message(FKX))
) -> list({ok, FKX} | {error, await_error()}).
...
-spec try_await_all(list(task(FKR)), integer()) -> list({ok, FKR} |
    {error, await_error()}).
