...
-type message(ECE) :: {message, ECE} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(ECF, ECG) :: {continue,
        ECG,
        gleam@option:option(gleam@erlang@process:selector(ECF))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(ECH, ECI) :: {ready, ECH, gleam@erlang@process:selector(ECI)} |
    {failed, binary()}.

-type self(ECJ, ECK) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        ECJ,
        gleam@erlang@process:subject(ECK),
        gleam@erlang@process:selector(message(ECK)),
        gleam@otp@system:debug_state(),
        fun((ECK, ECJ) -> next(ECK, ECJ))}.

-type spec(ECL, ECM) :: {spec,
        fun(() -> init_result(ECL, ECM)),
        integer(),
        fun((ECM, ECL) -> next(ECM, ECL))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(ECN) :: {ack,
        {ok, gleam@erlang@process:subject(ECN)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-file("src/gleam/otp/actor.gleam", 178).
-spec continue(ECU) -> next(any(), ECU).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(ECY, ECZ), gleam@erlang@process:selector(ECY)) -> next(ECY, ECZ).
...
-spec selecting_system_messages(gleam@erlang@process:selector(message(EDK))) -> gleam@erlang@process:selector(message(EDK)).
...
-spec receive_message(self(any(), EDG)) -> message(EDG).
...
-spec init_selector(
    gleam@erlang@process:subject(EIK),
    gleam@erlang@process:selector(EIK)
) -> gleam@erlang@process:selector(message(EIK)).
...
-spec initialise_actor(
    spec(any(), EEB),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(EEB)} |
        {error, gleam@erlang@process:exit_reason()})
...
-spec start_spec(spec(any(), EEO)) -> {ok, gleam@erlang@process:subject(EEO)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-spec start(EEU, fun((EEV, EEU) -> next(EEV, EEU))) -> {ok,
        gleam@erlang@process:subject(EEV)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec send(gleam@erlang@process:subject(EFB), EFB) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(EFD),
    fun((gleam@erlang@process:subject(EFF)) -> EFD),
    integer()
) -> EFF.
