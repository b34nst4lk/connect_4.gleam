...
-spec method_override(gleam@http@request:request(XTY)) -> gleam@http@request:request(XTY).
...
-spec fn_with_bad_request_error(fun((XUZ) -> {ok, XVA} | {error, any()})) -> fun((XUZ) -> {ok,
        XVA} |
    {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((XUT, bitstring()) -> {ok, XUT} |
        {error, gleam@http@response:response(body())}),
    XUT
) -> {ok, {gleam@option:option(buffered_reader()), integer(), XUT}} |
    {error, gleam@http@response:response(body())}.
...
-spec sort_keys(list({binary(), XVR})) -> list({binary(), XVR}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_400(
    {ok, XVU} | {error, any()},
    fun((XVU) -> gleam@http@response:response(body()))
) -> gleam@http@response:response(body()).
or_400(Result, Next) ->
    case Result of
        {ok, Value} ->
...
-spec or_500({ok, XUL} | {error, any()}) -> {ok, XUL} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
