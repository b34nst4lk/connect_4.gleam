...
-type message(KOH) :: {packet, bitstring()} | {user, KOH}.
...
-type connection(KOI) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(KOI))}.

-opaque handler(KOJ, KOK) :: {handler,
        glisten@socket@options:interface(),
        fun((connection(KOJ)) -> {KOK,
            gleam@option:option(gleam@erlang@process:selector(KOJ))}),
        fun((message(KOJ), KOK, connection(KOJ)) -> gleam@otp@actor:next(message(KOJ), KOK)),
        gleam@option:option(fun((KOK) -> nil)),
        integer(),
        boolean(),
        boolean()}.
...
-spec convert_on_init(
    fun((connection(KPY)) -> {KQA,
        gleam@option:option(gleam@erlang@process:selector(KPY))})
) -> fun((glisten@internal@handler:connection(KPY)) -> {KQA,
    gleam@option:option(gleam@erlang@process:selector(KPY))}).
...
-spec handler(
    fun((connection(KQG)) -> {KQI,
        gleam@option:option(gleam@erlang@process:selector(KQG))}),
    fun((message(KQG), KQI, connection(KQG)) -> gleam@otp@actor:next(message(KQG), KQI))
) -> handler(KQG, KQI).
handler(On_init, Loop) ->
    {handler, loopback, On_init, Loop, none, 10, false, false}.

-file("src/glisten.gleam", 188).
-spec map_user_selector(gleam@erlang@process:selector(message(KPN))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(KPN)).
...
-spec convert_loop(
    fun((message(KPS), KPT, connection(KPS)) -> gleam@otp@actor:next(message(KPS), KPT))
) -> fun((glisten@internal@handler:loop_message(KPS), KPT, glisten@internal@handler:connection(KPS)) -> gleam@otp@actor:next(glisten@internal@handler:loop_message(KPS), KPT)).
convert_loop(Loop) ->
    fun(Msg, Data, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec with_close(handler(KQP, KQQ), fun((KQQ) -> nil)) -> handler(KQP, KQQ).
...
-spec with_pool_size(handler(KQV, KQW), integer()) -> handler(KQV, KQW).
...
-spec with_http2(handler(KRB, KRC)) -> handler(KRB, KRC).
with_http2(Handler) ->
    _record = Handler,
    {handler,
        erlang:element(2, _record),
...
-spec with_ipv6(handler(KRN, KRO)) -> handler(KRN, KRO).
with_ipv6(Handler) ->
    _record = Handler,
    {handler,
        erlang:element(2, _record),
...
-spec bind(handler(KRH, KRI), binary()) -> handler(KRH, KRI).
