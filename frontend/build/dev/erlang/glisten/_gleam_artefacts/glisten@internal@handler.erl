...
-type message(JVH) :: {internal, internal_message()} | {user, JVH}.

-type loop_message(JVI) :: {packet, bitstring()} | {custom, JVI}.

-type loop_state(JVJ, JVK) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(JVJ)),
        glisten@transport:transport(),
        JVK}.

-type connection(JVL) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(JVL))}.

-type handler(JVM, JVN) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(JVM), JVN, connection(JVM)) -> gleam@otp@actor:next(loop_message(JVM), JVN)),
        fun((connection(JVM)) -> {JVN,
            gleam@option:option(gleam@erlang@process:selector(JVM))}),
        gleam@option:option(fun((JVN) -> nil)),
        glisten@transport:transport()}.

-file("src/glisten/internal/handler.gleam", 73).
?DOC(false).
-spec start(handler(JVX, any())) -> {ok,
        gleam@erlang@process:subject(message(JVX))} |
