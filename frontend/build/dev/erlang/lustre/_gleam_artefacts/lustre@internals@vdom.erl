...
-type attribute(LZP) :: {attribute,
        binary(),
        gleam@dynamic:dynamic_(),
        boolean()} |
    {event,
        binary(),
        fun((gleam@dynamic:dynamic_()) -> {ok, LZP} |
            {error, list(gleam@dynamic:decode_error())})}.

-file("src/lustre/internals/vdom.gleam", 508).
?DOC(false).
-spec attribute_to_event_handler(attribute(MBR)) -> {ok,
        {binary(),
            fun((gleam@dynamic:dynamic_()) -> {ok, MBR} |
                {error, list(gleam@dynamic:decode_error())})}} |
    {error, nil}.
...
-spec do_element_list_handlers(
    list(element(MAD)),
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, MAD} |
        {error, list(gleam@dynamic:decode_error())})),
    binary()
) -> gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, MAD} |
    {error, list(gleam@dynamic:decode_error())})).
...
-spec do_handlers(
    element(LZV),
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, LZV} |
        {error, list(gleam@dynamic:decode_error())})),
    binary()
) -> gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, LZV} |
    {error, list(gleam@dynamic:decode_error())})).
...
?DOC(false).
-spec handlers(element(LZQ)) -> gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok,
        LZQ} |
    {error, list(gleam@dynamic:decode_error())})).
handlers(Element) ->
