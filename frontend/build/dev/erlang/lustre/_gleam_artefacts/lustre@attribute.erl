...
-spec on(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, MLW} |
        {error, list(gleam@dynamic:decode_error())})
) -> lustre@internals@vdom:attribute(MLW).
on(Name, Handler) ->
...
-spec map(lustre@internals@vdom:attribute(MMB), fun((MMB) -> MMD)) -> lustre@internals@vdom:attribute(MMD).
map(Attr, F) ->
    case Attr of
