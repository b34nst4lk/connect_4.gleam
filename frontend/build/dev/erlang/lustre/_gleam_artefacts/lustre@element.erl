...
-spec element(
    binary(),
    list(lustre@internals@vdom:attribute(MUF)),
    list(lustre@internals@vdom:element(MUF))
) -> lustre@internals@vdom:element(MUF).
...
-spec do_keyed(lustre@internals@vdom:element(MUS), binary()) -> lustre@internals@vdom:element(MUS).
do_keyed(El, Key) ->
    case El of
...
-spec keyed(
    fun((list(lustre@internals@vdom:element(MUL))) -> lustre@internals@vdom:element(MUL)),
    list({binary(), lustre@internals@vdom:element(MUL)})
) -> lustre@internals@vdom:element(MUL).
keyed(El, Children) ->
    El(
        begin
            gleam@list:map(
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@internals@vdom:attribute(MUV)),
    list(lustre@internals@vdom:element(MUV))
) -> lustre@internals@vdom:element(MUV).
...
-spec advanced(
    binary(),
    binary(),
    list(lustre@internals@vdom:attribute(MVB)),
    list(lustre@internals@vdom:element(MVB)),
    boolean(),
    boolean()
) -> lustre@internals@vdom:element(MVB).
...
-spec fragment(list(lustre@internals@vdom:element(MVL))) -> lustre@internals@vdom:element(MVL).
fragment(Elements) ->
    element(
...
-spec map(lustre@internals@vdom:element(MVP), fun((MVP) -> MVR)) -> lustre@internals@vdom:element(MVR).
map(Element, F) ->
    case Element of
        {text, Content} ->
            {text, Content};
...
?DOC(false).
-spec get_root(fun((fun((MVT) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> lustre@effect:effect(MVT).
get_root(Effect) ->
