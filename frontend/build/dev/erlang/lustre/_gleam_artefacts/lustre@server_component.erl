...
-spec component(list(lustre@internals@vdom:attribute(QYB))) -> lustre@internals@vdom:element(QYB).
component(Attrs) ->
...
-spec subscribe(binary(), fun((lustre@internals@patch:patch(QYO)) -> nil)) -> lustre@internals@runtime:action(QYO, lustre:server_component()).
...
-spec do_select(
    fun((fun((QZC) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(QZC))
) -> lustre@effect:effect(QZC).
do_select(Sel) ->
...
-spec select(
    fun((fun((QYX) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(QYX))
) -> lustre@effect:effect(QYX).
select(Sel) ->
    do_select(Sel).
...
?DOC("\n").
-spec set_selector(
    gleam@erlang@process:selector(lustre@internals@runtime:action(any(), QZI))
) -> lustre@effect:effect(QZI).
set_selector(_) ->
    lustre@effect:from(
        fun(_) ->
            _pipe = <<"
