...
-type state(NTP, NTQ, NTR) :: {state,
        gleam@erlang@process:subject(action(NTQ, NTR)),
        gleam@erlang@process:selector(action(NTQ, NTR)),
        NTP,
        fun((NTP, NTQ) -> {NTP, lustre@effect:effect(NTQ)}),
        fun((NTP) -> lustre@internals@vdom:element(NTQ)),
        lustre@internals@vdom:element(NTQ),
        gleam@dict:dict(binary(), fun((lustre@internals@patch:patch(NTQ)) -> nil)),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, NTQ} |
            {error, list(gleam@dynamic:decode_error())})),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, NTQ} |
            {error, list(gleam@dynamic:decode_error())}))}.

-type action(NTS, NTT) :: {attrs, list({binary(), gleam@dynamic:dynamic_()})} |
    {batch, list(NTS), lustre@effect:effect(NTS)} |
    {debug, debug_action()} |
    {dispatch, NTS} |
    {emit, binary(), gleam@json:json()} |
    {event, binary(), gleam@dynamic:dynamic_()} |
    {set_selector, gleam@erlang@process:selector(NTS)} |
    shutdown |
    {subscribe, binary(), fun((lustre@internals@patch:patch(NTS)) -> nil)} |
    {unsubscribe, binary()} |
    {gleam_phantom, NTT}.
...
?DOC(false).
-spec run_renderers(
    gleam@dict:dict(any(), fun((lustre@internals@patch:patch(NUY)) -> nil)),
    lustre@internals@patch:patch(NUY)
) -> nil.
...
?DOC(false).
-spec run_effects(
    lustre@effect:effect(NVD),
    gleam@erlang@process:subject(action(NVD, any()))
) -> nil.
...
?DOC(false).
-spec loop(action(NUI, NUJ), state(NUM, NUI, NUJ)) -> gleam@otp@actor:next(action(NUI, NUJ), state(NUM, NUI, NUJ)).
loop(Message, State) ->
    case Message of
        {attrs, Attrs} ->
...
?DOC(false).
-spec start(
    {NTU, lustre@effect:effect(NTV)},
    fun((NTU, NTV) -> {NTU, lustre@effect:effect(NTV)}),
    fun((NTU) -> lustre@internals@vdom:element(NTV)),
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, NTV} |
        {error, list(gleam@dynamic:decode_error())}))
) -> {ok, gleam@erlang@process:subject(action(NTV, any()))} |
