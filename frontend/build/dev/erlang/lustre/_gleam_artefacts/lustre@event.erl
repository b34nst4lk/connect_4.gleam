...
-spec on(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, QSZ} |
        {error, list(gleam@dynamic:decode_error())})
) -> lustre@internals@vdom:attribute(QSZ).
on(Name, Handler) ->
    lustre@attribute:on(Name, Handler).

-file("src/lustre/event.gleam", 43).
?DOC("\n").
-spec on_click(QTC) -> lustre@internals@vdom:attribute(QTC).
on_click(Msg) ->
    on(<<"click"/utf8>>, fun(_) -> {ok, Msg} end).

-file("src/lustre/event.gleam", 49).
?DOC("\n").
-spec on_mouse_down(QTE) -> lustre@internals@vdom:attribute(QTE).
on_mouse_down(Msg) ->
    on(<<"mousedown"/utf8>>, fun(_) -> {ok, Msg} end).

-file("src/lustre/event.gleam", 55).
?DOC("\n").
-spec on_mouse_up(QTG) -> lustre@internals@vdom:attribute(QTG).
on_mouse_up(Msg) ->
    on(<<"mouseup"/utf8>>, fun(_) -> {ok, Msg} end).

-file("src/lustre/event.gleam", 61).
?DOC("\n").
-spec on_mouse_enter(QTI) -> lustre@internals@vdom:attribute(QTI).
on_mouse_enter(Msg) ->
    on(<<"mouseenter"/utf8>>, fun(_) -> {ok, Msg} end).

-file("src/lustre/event.gleam", 67).
?DOC("\n").
-spec on_mouse_leave(QTK) -> lustre@internals@vdom:attribute(QTK).
on_mouse_leave(Msg) ->
    on(<<"mouseleave"/utf8>>, fun(_) -> {ok, Msg} end).

-file("src/lustre/event.gleam", 73).
?DOC("\n").
-spec on_mouse_over(QTM) -> lustre@internals@vdom:attribute(QTM).
on_mouse_over(Msg) ->
    on(<<"mouseover"/utf8>>, fun(_) -> {ok, Msg} end).

-file("src/lustre/event.gleam", 79).
?DOC("\n").
-spec on_mouse_out(QTO) -> lustre@internals@vdom:attribute(QTO).
on_mouse_out(Msg) ->
...
-spec on_keypress(fun((binary()) -> QTQ)) -> lustre@internals@vdom:attribute(QTQ).
on_keypress(Msg) ->
...
-spec on_keydown(fun((binary()) -> QTS)) -> lustre@internals@vdom:attribute(QTS).
on_keydown(Msg) ->
...
-spec on_keyup(fun((binary()) -> QTU)) -> lustre@internals@vdom:attribute(QTU).
on_keyup(Msg) ->
...
-spec on_focus(QUC) -> lustre@internals@vdom:attribute(QUC).
on_focus(Msg) ->
    on(<<"focus"/utf8>>, fun(_) -> {ok, Msg} end).

-file("src/lustre/event.gleam", 154).
-spec on_blur(QUE) -> lustre@internals@vdom:attribute(QUE).
on_blur(Msg) ->
...
?DOC("\n").
-spec on_input(fun((binary()) -> QTW)) -> lustre@internals@vdom:attribute(QTW).
on_input(Msg) ->
...
-spec on_check(fun((boolean()) -> QTY)) -> lustre@internals@vdom:attribute(QTY).
on_check(Msg) ->
...
-spec on_submit(QUA) -> lustre@internals@vdom:attribute(QUA).
on_submit(Msg) ->
    on(
        <<"submit"/utf8>>,
        fun(Event) ->
