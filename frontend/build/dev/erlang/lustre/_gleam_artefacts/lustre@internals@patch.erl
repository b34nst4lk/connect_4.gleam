...
-type patch(MZH) :: {diff, element_diff(MZH)} |
    {emit, binary(), gleam@json:json()} |
    {init, list(binary()), lustre@internals@vdom:element(MZH)}.

-type element_diff(MZI) :: {element_diff,
        gleam@dict:dict(binary(), lustre@internals@vdom:element(MZI)),
        gleam@set:set(binary()),
        gleam@dict:dict(binary(), attribute_diff(MZI)),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, MZI} |
            {error, list(gleam@dynamic:decode_error())}))}.

-type attribute_diff(MZJ) :: {attribute_diff,
        gleam@set:set(lustre@internals@vdom:attribute(MZJ)),
        gleam@set:set(binary()),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, MZJ} |
            {error, list(gleam@dynamic:decode_error())}))}.
...
-spec do_attribute(
    attribute_diff(NAI),
    binary(),
    {ok, lustre@internals@vdom:attribute(NAI)} | {error, nil},
    {ok, lustre@internals@vdom:attribute(NAI)} | {error, nil}
) -> attribute_diff(NAI).
...
?DOC(false).
-spec zip(list(NAZ), list(NAZ)) -> list({gleam@option:option(NAZ),
    gleam@option:option(NAZ)}).
zip(Xs, Ys) ->
    case {Xs, Ys} of
        {[], []} ->
            [];
...
?DOC(false).
-spec event_handler(lustre@internals@vdom:attribute(NBL)) -> {ok,
        {binary(),
            fun((gleam@dynamic:dynamic_()) -> {ok, NBL} |
                {error, list(gleam@dynamic:decode_error())})}} |
    {error, nil}.
...
?DOC(false).
-spec attribute_dict(list(lustre@internals@vdom:attribute(NBF))) -> gleam@dict:dict(binary(), lustre@internals@vdom:attribute(NBF)).
...
?DOC(false).
-spec attributes(
    list(lustre@internals@vdom:attribute(NAC)),
    list(lustre@internals@vdom:attribute(NAC))
) -> attribute_diff(NAC).
attributes(Old, New) ->
...
-spec do_element_list(
    element_diff(MZV),
    list(lustre@internals@vdom:element(MZV)),
    list(lustre@internals@vdom:element(MZV)),
    binary()
) -> element_diff(MZV).
...
-spec do_elements(
    element_diff(MZO),
    gleam@option:option(lustre@internals@vdom:element(MZO)),
    gleam@option:option(lustre@internals@vdom:element(MZO)),
    binary()
) -> element_diff(MZO).
...
?DOC(false).
-spec elements(
    lustre@internals@vdom:element(MZK),
    lustre@internals@vdom:element(MZK)
) -> element_diff(MZK).
elements(Old, New) ->
    do_elements(
...
-spec fold_element_list_event_handlers(
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, NBY} |
        {error, list(gleam@dynamic:decode_error())})),
    list(lustre@internals@vdom:element(NBY)),
    binary()
) -> gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, NBY} |
    {error, list(gleam@dynamic:decode_error())})).
...
-spec fold_event_handlers(
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, NBQ} |
        {error, list(gleam@dynamic:decode_error())})),
    lustre@internals@vdom:element(NBQ),
    binary()
) -> gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, NBQ} |
    {error, list(gleam@dynamic:decode_error())})).
