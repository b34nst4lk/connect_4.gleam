...
-opaque app(OPW, OPX, OPY) :: {app,
        fun((OPW) -> {OPX, lustre@effect:effect(OPY)}),
        fun((OPX, OPY) -> {OPX, lustre@effect:effect(OPY)}),
        fun((OPX) -> lustre@internals@vdom:element(OPY)),
...
-spec application(
    fun((OQR) -> {OQS, lustre@effect:effect(OQT)}),
    fun((OQS, OQT) -> {OQS, lustre@effect:effect(OQT)}),
    fun((OQS) -> lustre@internals@vdom:element(OQT))
) -> app(OQR, OQS, OQT).
...
-spec element(lustre@internals@vdom:element(OQF)) -> app(nil, nil, OQF).
element(Html) ->
...
-spec simple(
    fun((OQK) -> OQL),
    fun((OQL, OQM) -> OQL),
    fun((OQL) -> lustre@internals@vdom:element(OQM))
) -> app(OQK, OQL, OQM).
...
-spec component(
    fun((ORA) -> {ORB, lustre@effect:effect(ORC)}),
    fun((ORB, ORC) -> {ORB, lustre@effect:effect(ORC)}),
    fun((ORB) -> lustre@internals@vdom:element(ORC)),
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, ORC} |
        {error, list(gleam@dynamic:decode_error())}))
) -> app(ORA, ORB, ORC).
...
-spec do_start(app(ORW, any(), ORY), binary(), ORW) -> {ok,
        fun((lustre@internals@runtime:action(ORY, client_spa())) -> nil)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.

-file("src/lustre.gleam", 444).
-spec do_start_actor(app(OTB, any(), OTD), OTB) -> {ok,
        gleam@erlang@process:subject(lustre@internals@runtime:action(OTD, server_component()))} |
    {error, error()}.
...
-spec start_actor(app(OSQ, any(), OSS), OSQ) -> {ok,
        gleam@erlang@process:subject(lustre@internals@runtime:action(OSS, server_component()))} |
    {error, error()}.
...
-spec start_server_component(app(OSG, any(), OSI), OSG) -> {ok,
        fun((lustre@internals@runtime:action(OSI, server_component())) -> nil)} |
    {error, error()}.
...
-spec dispatch(OTT) -> lustre@internals@runtime:action(OTT, any()).
dispatch(Msg) ->
    {dispatch, Msg}.
...
-spec start(app(ORM, any(), ORO), binary(), ORM) -> {ok,
        fun((lustre@internals@runtime:action(ORO, client_spa())) -> nil)} |
    {error, error()}.
