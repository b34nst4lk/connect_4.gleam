...
-opaque effect(LVA) :: {effect, list(fun((actions(LVA)) -> nil))}.

-type actions(LVB) :: {actions,
        fun((LVB) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(LVB)) -> nil),
        gleam@dynamic:dynamic_()}.

-file("src/lustre/effect.gleam", 126).
?DOC(false).
-spec custom(
    fun((fun((LVG) -> nil), fun((binary(), gleam@json:json()) -> nil), fun((gleam@erlang@process:selector(LVG)) -> nil), gleam@dynamic:dynamic_()) -> nil)
) -> effect(LVG).
custom(Run) ->
    {effect,
        [fun(Actions) ->
...
-spec from(fun((fun((LVC) -> nil)) -> nil)) -> effect(LVC).
from(Effect) ->
...
-spec batch(list(effect(LVL))) -> effect(LVL).
batch(Effects) ->
    {effect,
        begin
            gleam@list:fold(
...
-spec map(effect(LVP), fun((LVP) -> LVR)) -> effect(LVR).
map(Effect, F) ->
    {effect,
        begin
            gleam@list:map(
...
-spec perform(
    ...
    fun((LVT) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(LVT)) -> nil),
    gleam@dynamic:dynamic_()
) -> nil.
