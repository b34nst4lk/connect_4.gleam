...
-file("src/tom.gleam", 383).
-spec push_key({ok, UXF} | {error, get_error()}, binary()) -> {ok, UXF} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-file("src/tom.gleam", 564).
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {UZK, list(binary())}} | {error, parse_error()})
) -> {ok, {UZK, list(binary())}} | {error, parse_error()}.
...
-file("src/tom.gleam", 708).
-spec do(
    {ok, {UZV, list(binary())}} | {error, parse_error()},
    fun((UZV, list(binary())) -> {ok, UZY} | {error, parse_error()})
) -> {ok, UZY} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-file("src/tom.gleam", 718).
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {VAD, list(binary())}} | {error, parse_error()})
) -> {ok, {VAD, list(binary())}} | {error, parse_error()}.
