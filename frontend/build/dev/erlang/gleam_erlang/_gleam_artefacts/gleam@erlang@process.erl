...
-opaque subject(CDU) :: {subject, pid_(), gleam@erlang:reference_()} |
    {gleam_phantom, CDU}.

-type do_not_leak() :: any().

-type selector(CDV) :: any() | {gleam_phantom, CDV}.
...
-type call_error(CDW) :: {callee_down, gleam@dynamic:dynamic_()} |
    call_timeout |
    {gleam_phantom, CDW}.
...
-spec 'receive'(subject(CEH), integer()) -> {ok, CEH} | {error, nil}.
...
-spec receive_forever(subject(CEL)) -> CEL.
...
-spec select(selector(CEP), integer()) -> {ok, CEP} | {error, nil}.
select(From, Within) ->
...
-spec select_forever(selector(CET)) -> CET.
select_forever(From) ->
...
-spec map_selector(selector(CEV), fun((CEV) -> CEX)) -> selector(CEX).
map_selector(A, B) ->
...
-spec merge_selector(selector(CEZ), selector(CEZ)) -> selector(CEZ).
merge_selector(A, B) ->
...
-spec selecting_trapped_exits(selector(CFD), fun((exit_message()) -> CFD)) -> selector(CFD).
...
-spec selecting(selector(CFG), subject(CFI), fun((CFI) -> CFG)) -> selector(CFG).
...
-spec selecting_record2(
    selector(CFQ),
    any(),
    fun((gleam@dynamic:dynamic_()) -> CFQ)
) -> selector(CFQ).
...
-spec selecting_record3(
    selector(CFU),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> CFU)
) -> selector(CFU).
...
-spec selecting_record4(
    selector(CFY),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> CFY)
) -> selector(CFY).
...
-spec selecting_record5(
    selector(CGC),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> CGC)
) -> selector(CGC).
...
-spec selecting_record6(
    selector(CGG),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> CGG)
) -> selector(CGG).
...
-spec selecting_record7(
    selector(CGK),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> CGK)
) -> selector(CGK).
...
-spec selecting_record8(
    selector(CGO),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> CGO)
) -> selector(CGO).
...
-spec selecting_anything(selector(CGS), fun((gleam@dynamic:dynamic_()) -> CGS)) -> selector(CGS).
...
-spec deselecting(selector(CFL), subject(any())) -> selector(CFL).
...
-spec deselecting_process_down(selector(CHH), process_monitor()) -> selector(CHH).
...
-spec try_call(subject(CHK), fun((subject(CHM)) -> CHK), integer()) -> {ok, CHM} |
    {error, call_error(CHM)}.
...
-spec call(subject(CHR), fun((subject(CHT)) -> CHR), integer()) -> CHT.
...
-spec try_call_forever(subject(CHZ), fun((subject(CIB)) -> CHZ)) -> {ok, CIB} |
...
-spec call_forever(subject(CHV), fun((subject(CHX)) -> CHV)) -> CHX.
...
-spec send_after(subject(CII), integer(), CII) -> timer().
