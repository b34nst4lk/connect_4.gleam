...
-type action(GIZ) :: stop | {continue, GIZ, fun(() -> action(GIZ))}.

-opaque yielder(GJA) :: {yielder, fun(() -> action(GJA))}.

-type step(GJB, GJC) :: {next, GJB, GJC} | done.

-type chunk(GJD, GJE) :: {another_by,
        list(GJD),
        GJE,
        GJD,
        fun(() -> action(GJD))} |
...
-type sized_chunk(GJF) :: {another, list(GJF), fun(() -> action(GJF))} |
    {last, list(GJF)} |
    no_more.
...
-spec unfold_loop(GJN, fun((GJN) -> step(GJO, GJN))) -> fun(() -> action(GJO)).
...
-spec unfold(GJI, fun((GJI) -> step(GJJ, GJI))) -> yielder(GJJ).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> GJS)) -> yielder(GJS).
repeatedly(F) ->
...
-spec from_list(list(GJW)) -> yielder(GJW).
from_list(List) ->
...
-spec transform(yielder(GKG), GKI, fun((GKI, GKG) -> step(GKJ, GKI))) -> yielder(GKJ).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 204).
-spec fold_loop(fun(() -> action(GKQ)), fun((GKS, GKQ) -> GKS), GKS) -> GKS.
...
-spec fold(yielder(GKN), GKP, fun((GKP, GKN) -> GKP)) -> GKP.
...
-spec to_list(yielder(GKV)) -> list(GKV).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(GKY)) -> step(GKY, yielder(GKY)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(GLG)), integer()) -> fun(() -> action(GLG)).
...
-spec take(yielder(GLD), integer()) -> yielder(GLD).
...
-spec drop_loop(fun(() -> action(GLM)), integer()) -> action(GLM).
...
-spec map(yielder(GLP), fun((GLP) -> GLR)) -> yielder(GLR).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(GLX), yielder(GLZ), fun((GLX, GLZ) -> GMB)) -> yielder(GMB).
...
-spec append_loop(fun(() -> action(GMN)), fun(() -> action(GMN))) -> action(GMN).
...
-spec append(yielder(GMJ), yielder(GMJ)) -> yielder(GMJ).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(GMV)))) -> action(GMV).
...
-spec flatten(yielder(yielder(GMR))) -> yielder(GMR).
flatten(Yielder) ->
...
-spec concat(list(yielder(GMZ))) -> yielder(GMZ).
concat(Yielders) ->
...
-spec flat_map(yielder(GND), fun((GND) -> yielder(GNF))) -> yielder(GNF).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(GNL)), fun((GNL) -> boolean())) -> action(GNL).
...
-spec filter(yielder(GNI), fun((GNI) -> boolean())) -> yielder(GNI).
...
-spec filter_map(yielder(GNO), fun((GNO) -> {ok, GNQ} | {error, any()})) -> yielder(GNQ).
...
-spec cycle(yielder(GOC)) -> yielder(GOC).
cycle(Yielder) ->
    _pipe = repeat(Yielder),
    flatten(_pipe).

-file("src/gleam/yielder.gleam", 709).
-spec find_loop(fun(() -> action(GOK)), fun((GOK) -> boolean())) -> {ok, GOK} |
    {error, nil}.
...
-spec find(yielder(GOG), fun((GOG) -> boolean())) -> {ok, GOG} | {error, nil}.
...
-spec find_map(yielder(GOO), fun((GOO) -> {ok, GOQ} | {error, any()})) -> {ok,
        GOQ} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(GPH)), integer()) -> fun(() -> action({GPH,
    integer()})).
...
-spec index(yielder(GPE)) -> yielder({GPE, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(GPK, fun((GPK) -> GPK)) -> yielder(GPK).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src/gleam/yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(GPP)), fun((GPP) -> boolean())) -> fun(() -> action(GPP)).
...
-spec take_while(yielder(GPM), fun((GPM) -> boolean())) -> yielder(GPM).
...
-spec drop_while_loop(fun(() -> action(GPV)), fun((GPV) -> boolean())) -> action(GPV).
...
-spec drop_while(yielder(GPS), fun((GPS) -> boolean())) -> yielder(GPS).
...
-spec scan_loop(fun(() -> action(GQC)), fun((GQE, GQC) -> GQE), GQE) -> fun(() -> action(GQE)).
...
-spec scan(yielder(GPY), GQA, fun((GQA, GPY) -> GQA)) -> yielder(GQA).
...
-spec zip_loop(fun(() -> action(GQL)), fun(() -> action(GQN))) -> fun(() -> action({GQL,
    GQN})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(GQG), yielder(GQI)) -> yielder({GQG, GQI}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(GRA)), fun((GRA) -> GRC), GRC, list(GRA)) -> chunk(GRA, GRC).
...
-spec chunk_loop(fun(() -> action(GQV)), fun((GQV) -> GQX), GQX, GQV) -> action(list(GQV)).
...
-spec chunk(yielder(GQQ), fun((GQQ) -> any())) -> yielder(list(GQQ)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(GRO)), integer(), list(GRO)) -> sized_chunk(GRO).
...
-spec sized_chunk_loop(fun(() -> action(GRK)), integer()) -> fun(() -> action(list(GRK))).
...
-spec sized_chunk(yielder(GRG), integer()) -> yielder(list(GRG)).
...
-spec intersperse_loop(fun(() -> action(GRV)), GRV) -> action(GRV).
...
-spec intersperse(yielder(GRS), GRS) -> yielder(GRS).
...
-spec any_loop(fun(() -> action(GSA)), fun((GSA) -> boolean())) -> boolean().
...
-spec any(yielder(GRY), fun((GRY) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1228).
-spec all_loop(fun(() -> action(GSE)), fun((GSE) -> boolean())) -> boolean().
...
-spec all(yielder(GSC), fun((GSC) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1273).
-spec update_group_with(GSU) -> fun((gleam@option:option(list(GSU))) -> list(GSU)).
...
-spec group_updater(fun((GSM) -> GSN)) -> fun((gleam@dict:dict(GSN, list(GSM)), GSM) -> gleam@dict:dict(GSN, list(GSM))).
group_updater(F) ->
...
-spec group(yielder(GSG), fun((GSG) -> GSI)) -> gleam@dict:dict(GSI, list(GSG)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(GSY), fun((GSY, GSY) -> GSY)) -> {ok, GSY} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(GTC)) -> {ok, GTC} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(GTK) -> yielder(GTK).
single(Elem) ->
    once(fun() -> Elem end).

-file("src/gleam/yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(GTQ)), fun(() -> action(GTQ))) -> action(GTQ).
...
-spec interleave(yielder(GTM), yielder(GTM)) -> yielder(GTM).
...
-spec fold_until_loop(
    fun(() -> action(GTY)),
    fun((GUA, GTY) -> gleam@list:continue_or_stop(GUA)),
    GUA
) -> GUA.
...
-spec fold_until(
    yielder(GTU),
    GTW,
    fun((GTW, GTU) -> gleam@list:continue_or_stop(GTW))
) -> GTW.
...
-spec try_fold(yielder(GUC), GUE, fun((GUE, GUC) -> {ok, GUE} | {error, GUF})) -> {ok,
        GUE} |
    {error, GUF}.
...
-spec first(yielder(GUS)) -> {ok, GUS} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(GUW), integer()) -> {ok, GUW} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(GVE), fun((GVE) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(GVH, fun(() -> yielder(GVH))) -> yielder(GVH).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(GVK), GVK) -> yielder(GVK).
