...
-spec one_of(list(HQU)) -> fun((HQU) -> {ok, HQU} | {error, snag:snag()}).
one_of(Allowed) ->
...
-spec none_of(list(HQX)) -> fun((HQX) -> {ok, HQX} | {error, snag:snag()}).
none_of(Disallowed) ->
...
-spec each(fun((HRA) -> {ok, HRA} | {error, snag:snag()})) -> fun((list(HRA)) -> {ok,
        list(HRA)} |
    {error, snag:snag()}).
each(Constraint) ->
