...
-opaque glint(HXW) :: {glint, config(), command_node(HXW)}.

-type args_count() :: {eq_args, integer()} | {min_args, integer()}.

-opaque command(HXX) :: {command,
        fun((named_args(), list(binary()), flags()) -> HXX),
        flags(),
        binary(),
        gleam@option:option(args_count()),
        list(binary())}.

-type internal_command(HXY) :: {internal_command,
        fun((named_args(), list(binary()), flags()) -> HXY),
        flags(),
        gleam@option:option(args_count()),
        list(binary())}.
...
-type command_node(HXZ) :: {command_node,
        gleam@option:option(internal_command(HXZ)),
        gleam@dict:dict(binary(), command_node(HXZ)),
        flags(),
        binary()}.

-type out(HYA) :: {out, HYA} | {help, binary()}.
...
-opaque flag(HYB) :: {flag,
        binary(),
        binary(),
        fun((binary()) -> {ok, HYB} | {error, snag:snag()}),
        fun((flag_internals(HYB)) -> value()),
        fun((flags(), binary()) -> {ok, HYB} | {error, snag:snag()}),
        gleam@option:option(HYB)}.

-type flag_internals(HYC) :: {flag_internals,
        gleam@option:option(HYC),
        fun((binary()) -> {ok, HYC} | {error, snag:snag()})}.
...
-spec pretty_help(glint(HYJ), pretty_help()) -> glint(HYJ).
...
-spec with_name(glint(HYM), binary()) -> glint(HYM).
...
-spec without_exit(glint(HYP)) -> glint(HYP).
without_exit(Glint) ->
    _record = Glint,
    {glint,
        begin
...
-spec as_module(glint(HYS)) -> glint(HYS).
as_module(Glint) ->
    _record = Glint,
    {glint,
        begin
...
-spec with_indent_width(glint(HYV), integer()) -> glint(HYV).
...
-spec with_max_output_width(glint(HYY), integer()) -> glint(HYY).
...
-spec with_min_first_column_width(glint(HZB), integer()) -> glint(HZB).
...
-spec with_column_gap(glint(HZE), integer()) -> glint(HZE).
...
-spec global_help(glint(HZN), binary()) -> glint(HZN).
...
-spec map_command(command(IAC), fun((IAC) -> IAE)) -> command(IAE).
...
-spec command_help(binary(), fun(() -> command(IAG))) -> command(IAG).
command_help(Desc, F) ->
    _record = F(),
    {command,
...
-spec unnamed_args(args_count(), fun(() -> command(IAJ))) -> command(IAJ).
...
-spec named_arg(
    binary(),
    fun((fun((named_args()) -> binary())) -> command(IAM))
) -> command(IAM).
named_arg(Name, F) ->
    Cmd = begin
        F(
...
-spec new_builder(
    binary(),
    fun((flag_internals(IDA)) -> value()),
    fun((flags(), binary()) -> {ok, IDA} | {error, snag:snag()}),
    fun((binary()) -> {ok, IDA} | {error, snag:snag()})
) -> flag(IDA).
...
-spec wrap_with_constraint(
    fun((binary()) -> {ok, IDM} | {error, snag:snag()}),
    fun((IDM) -> {ok, IDM} | {error, snag:snag()})
) -> fun((binary()) -> {ok, IDM} | {error, snag:snag()}).
...
-spec flag_constraint(flag(IDI), fun((IDI) -> {ok, IDI} | {error, snag:snag()})) -> flag(IDI).
...
-spec flag_help(flag(IEB), binary()) -> flag(IEB).
...
-spec flag_default(flag(IEE), IEE) -> flag(IEE).
...
-spec flag(
    flag(IAP),
    fun((fun((flags()) -> {ok, IAP} | {error, snag:snag()})) -> command(IAS))
) -> command(IAS).
flag(Flag, F) ->
    Cmd = F(
        fun(_capture) ->
...
-spec fold(flags(), IEH, fun((IEH, binary(), flag_entry()) -> IEH)) -> IEH.
fold(Flags, Acc, F) ->
...
-spec command(fun((named_args(), list(binary()), flags()) -> HZZ)) -> command(HZZ).
command(Runner) ->
...
-spec get_value(
    flags(),
    binary(),
    fun((flag_entry()) -> {ok, IEU} | {error, snag:snag()})
) -> {ok, IEU} | {error, snag:snag()}.
...
-spec get_flag(flags(), flag(IEX)) -> {ok, IEX} | {error, snag:snag()}.
get_flag(Flags, Flag) ->
...
-spec do_update_at(
    command_node(IFQ),
    list(binary()),
    fun((command_node(IFQ)) -> command_node(IFQ))
) -> command_node(IFQ).
...
-spec update_at(
    glint(IFK),
    list(binary()),
    fun((command_node(IFK)) -> command_node(IFK))
) -> glint(IFK).
...
-spec path_help(glint(HZJ), list(binary()), binary()) -> glint(HZJ).
...
-spec add(glint(HZQ), list(binary()), command(HZQ)) -> glint(HZQ).
...
-spec group_flag(glint(IAV), list(binary()), flag(any())) -> glint(IAV).
...
?DOC(false).
-spec execute(glint(IBB), list(binary())) -> {ok, out(IBB)} | {error, binary()}.
execute(Glint, Args) ->
    Help_flag = <<"--"/utf8,
        (erlang:element(
            2,
...
-spec run_and_handle(glint(ICA), list(binary()), fun((ICA) -> any())) -> nil.
