...
-opaque deque(BVC) :: {deque, list(BVC), list(BVC)}.
...
-spec from_list(list(BVF)) -> deque(BVF).
from_list(List) ->
    {deque, [], List}.
...
-spec to_list(deque(BVI)) -> list(BVI).
to_list(Deque) ->
...
-spec push_back(deque(BVP), BVP) -> deque(BVP).
...
-spec push_front(deque(BVS), BVS) -> deque(BVS).
...
-spec pop_back(deque(BVV)) -> {ok, {BVV, deque(BVV)}} | {error, nil}.
pop_back(Deque) ->
    case Deque of
        {deque, [], []} ->
            {error, nil};
...
-spec pop_front(deque(BWA)) -> {ok, {BWA, deque(BWA)}} | {error, nil}.
pop_front(Deque) ->
    case Deque of
        {deque, [], []} ->
            {error, nil};
...
-spec reverse(deque(BWF)) -> deque(BWF).
reverse(Deque) ->
...
-spec is_logically_equal(deque(BWI), deque(BWI), fun((BWI, BWI) -> boolean())) -> boolean().
...
-spec is_equal(deque(BWQ), deque(BWQ)) -> boolean().
is_equal(A, B) ->
    check_equal(
        erlang:element(3, A),
        erlang:element(2, A),
