...
-export_type([error/0, new/0, accepted/0, snapshot/1, outcome/0, info_line/0, split/0, command/0, review_choice/0]).
...
to_accepted_path(File) ->
    <<(filepath:strip_extension(File))/binary, ".accepted"/utf8>>.
...
pretty_box(Title, Content_lines, Info_lines) ->
    ...
    _assert_subject = begin
        ...
        gleam@result:'try'(
            ...
            fun(Digits) -> {ok, (erlang:length(Digits) * 2) + 5} end
        )
    end,
...
new_snapshot_box(Snapshot, Additional_info_lines) ->
    ...
    pretty_box(
        ...
        Content@1,
        gleam@list:flatten(
            [snapshot_default_lines(Snapshot), Additional_info_lines]
        )
    ).

-file("src/birdie.gleam", 576).
...
do_review(New_snapshot_paths, Titles, Current, Out_of) ->
...
suggest_run_command(Invalid, Command) ->
    ...
    case gleam_erlang_ffi:get_line(Msg) of
        {error, _} ->
            nil;
